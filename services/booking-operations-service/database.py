"""
Database configuration for Booking Operations Service
"""
...
def initialize_database():
    ...
    try:
        ...
        # Note: Actual table creation is handled by schema_manager
        ...
        with get_tenant_session(schema_name) as db:
            ...
            result = db.execute(text("""
                SELECT
                    ...
                    is_blocked,
                    block_reason
                FROM service_daily_capacity
                WHERE service_id = :service_id
                AND capacity_date = :date
            """), {
                "service_id": service_id,
                "date": date
            })

            capacity = result.first()

            if not capacity:
                # No specific capacity record, check general availability
                return {
                    "available": True,
                    "capacity": None,
                    "message": "No capacity restrictions"
                }

            if not capacity[3]:  # is_available
                return {
                    "available": False,
                    "reason": "Service not available on this date"
                }

            if capacity[4]:  # is_blocked
                return {
                    "available": False,
                    "reason": capacity[5] or "Service blocked on this date"
                }

            available = capacity[2]  # available_capacity
            if available >= passengers:
                ...
                return {
                    "available": False,
                    "reason": f"Insufficient capacity. Only {available} spots available.",
                    "capacity": {
                        "total": capacity[0],
                        "used": capacity[1],
                        "available": available
