...
async def get_current_tenant(
    ...
    """Extract current tenant information from user"""
    if not current_user:
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
    ...
    tenant_info = {
        ...
        "schema_name": f"tenant_{current_user.get('tenant_id')}" if current_user.get("tenant_id") else None
    }

    if not tenant_info["tenant_id"]:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="No tenant information available"
        )
